...
def generate_embedding_inputs(json_path: str, output_path: str = None):
    """
    从 true.json 生成嵌入模型的输入数据

    Args:
        json_path: true.json 文件路径
...
if __name__ == '__main__':
    ...
    # 文件路径
    json_path = os.path.join(script_dir, 'data', 'final_dataset.json')
    output_path = os.path.join(script_dir, 'data', 'embedding_inputs.json')

    # 生成嵌入输入
    embedding_inputs = generate_embedding_inputs(str(Path(__file__).parent.parent / "data" / "final_dataset.json"), str(Path(__file__).parent.parent / "data" / "embedding_inputs.json"))
